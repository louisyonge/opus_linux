...
//            System.loadLibrary("opusTool");
            System.load(currentDir + "/libs/libopusTool.so");
        } catch (Exception e) {
            System.out.println(TAG + ":loaded library opusTool failed!");
        }
    }
    ...
    /**
     * encode WAV data from a buffer and save it to opusFile
     * @param bufferIn WAV data
     * @param opusFileOut OutPut file
     * @param option Option for encoding
     * @return zero if successful.
     */
    public native int encode(ByteBuffer bufferIn, String opusFileOut, String option);

    /**
     * decode to a stream in memory
     * @param opusFile opusFile to be decoded
     * @param option option for decoding
     * @return decoded WAV data
     */
    public native ByteBuffer decode(String opusFile, String option);

    /**
     * release a ByteBuffer from Native level
     * @param buffer
     */
    public native void releaseByteBuffer(ByteBuffer buffer);
    /**
