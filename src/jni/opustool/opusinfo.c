		...
		case 3:
			if (!((val[j] == 0xE0 && val[j + 1] >= 0xA0 && val[j + 1] <= 0xBF
					...
					|| (val[j] == 0xED && val[j + 1] >= 0x80
							&& val[j + 1] <= 0x9F && (val[j + 2] & 0xC0) == 0x80)
			...
			break;
		case 4:
			if (!((val[j] == 0xF0 && val[j + 1] >= 0x90 && val[j + 1] <= 0xBF
					&& (val[j + 2] & 0xC0) == 0x80
					&& (val[j + 3] & 0xC0) == 0x80)
					...
					|| (val[j] == 0xF4 && val[j + 1] >= 0x80
							&& val[j + 1] <= 0x8F && (val[j + 2] & 0xC0) == 0x80
							&& (val[j + 3] & 0xC0) == 0x80)))
				broken = 1;
	...
	if (sep - comment == 22
		...
		if (mime_type_length == 10
			...
			format = PIC_FORMAT_GIF;
				...
				&& strncmp((const char *) data + 8, "-->", mime_type_length)
						== 0) {
			is_url = 1;
			/*TODO: validate URL.*/
		...
		case PIC_FORMAT_JPEG:
			extract_jpeg_params(data + j, image_length, &file_width,
					&file_height, &file_depth, &file_colors, &has_palette);
			break;
		case PIC_FORMAT_PNG:
			extract_png_params(data + j, image_length, &file_width,
					&file_height, &file_depth, &file_colors, &has_palette);
			break;
		case PIC_FORMAT_GIF:
			extract_gif_params(data + j, image_length, &file_width,
					&file_height, &file_depth, &file_colors, &has_palette);
			break;
		}
		...
		oi_info("\t%.*s%u|%.*s|%.*s|%ux%ux%u", sep + 1 - comment, comment,
				(unsigned) picture_type, mime_type_length, data + 8,
...
static void free_stream_set(stream_set *set) {
	...
	for (i = 0; i < set->used; i++) {
		if (!set->streams[i].end) {
			oi_warn(
					_(
							"WARNING: EOS not set on stream %d (normal for live streams)\n"),
					set->streams[i].num);
...
static stream_processor *find_stream_processor(stream_set *set, ogg_page *page) {
	...
	if (set->allocated < set->used)
		...
		set->streams = realloc(set->streams,
				sizeof(stream_processor) * set->allocated);
		stream = &set->streams[set->used];
	}
	set->used++;
...

	{
		...
		if (res <= 0) {
			...
			has_oi_supported = 1;
				...
				&& memcmp(packet.packet, "\x80theora", 7) == 0) {
			...
			other_start(stream, "speex");
				...
				&& memcmp(packet.packet, "fishead\0", 8) == 0)
			...
			other_start(stream, "dirac (legacy)");
				...
				&& memcmp(packet.packet, "\x80kate\0\0\0", 8) == 0) {
		...
		if (has_oi_supported)
			oi_info(_("Use ogginfo for more information on this file.\n"));
...
static int get_next_page(FILE *f, ogg_sync_state *ogsync, ogg_page *page,
	...
	while ((ret = ogg_sync_pageseek(ogsync, page)) <= 0) {
		if (ret < 0) {
			...
			oi_warn(
					_(
							"WARNING: Hole in data (%d bytes) found at approximate offset %" I64FORMAT " bytes. Corrupted Ogg.\n"),
					-ret, *written);
			continue;
		}
